...
- `russia-regions.json` — контуры 83 субъектов РФ уже в проекции Альберса (параллели 52° и 64°,
  ...
  (`assets/ru-regions-lite.geojson`, свойства `shapeISO`/`name`). Крым, Севастополь, Донецк и Луганск
  (коды CR, SEV, DON, LUG) — из geoBoundaries UKR ADM1 (CC BY 4.0), упрощены; их рисует DPI//CHECKER.
