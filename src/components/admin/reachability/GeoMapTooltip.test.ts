...
describe('tooltipPlacement', () => {
  it('в просторе — справа-снизу от курсора', () => {
    expect(tooltipPlacement({ x: 100, y: 100, width: 1000, height: 500 }, 1)).toEqual({
      left: 114,
      top: 114,
      maxWidth: 260,
    });
  ...
  it('у правого и нижнего края — прижимается к краю, не уходя за него', () => {
    const placed = tooltipPlacement({ x: 950, y: 480, width: 1000, height: 500 }, 3);
    expect(placed.left).toBe(1000 - 260 - 6);
    expect(placed.top + 48 + 22 * 3).toBeLessThanOrEqual(500 - 6);
    expect(placed.top).toBeGreaterThanOrEqual(6);
  });
  it('на карте телефона подсказка не вылезает ни вправо, ни вниз', () => {
    const placed = tooltipPlacement({ x: 60, y: 150, width: 330, height: 180 }, 1);
    expect(placed.maxWidth).toBe(260);
  ...
  it('карта уже подсказки — подсказка сужается до карты', () => {
    const placed = tooltipPlacement({ x: 10, y: 10, width: 200, height: 120 }, 1);
