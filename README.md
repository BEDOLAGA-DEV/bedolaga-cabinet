...
Название и логотип из админки бота кабинет подтягивает сам, ещё до загрузки
приложения (запросом `/cabinet/branding` из шапки страницы), поэтому готовый
образ с зашитым `Cabinet` показывает ваш бренд на вкладке и ярлыках без
пересборки. `VITE_APP_NAME` / `VITE_APP_LOGO` — только значения на те доли
секунды, пока API не ответил.
